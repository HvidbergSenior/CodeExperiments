...
export const useAddUser = (userData?: AllUserAdminResponse) => {
  ...
  const mapPossibleAndGivenCustomerPermissions = useCallback(
    (
      ...
      const access: AccessNode[] = [];
      if (selectedNodesOutputList !== undefined) {
  ...
  const filterCustomers = useCallback(
    (filter: string, baseCustomers?: TreeNode[]) => {
      let mutableBaseCustomers = baseCustomers ? [...baseCustomers] : undefined;
      if (mutableBaseCustomers === undefined) {
        mutableBaseCustomers = customers;
      }
      if (filter.trim() === "") {
