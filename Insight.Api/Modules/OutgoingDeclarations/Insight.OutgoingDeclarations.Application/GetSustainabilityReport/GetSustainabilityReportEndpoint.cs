...
{
    public static class GetSustainabilityReportEndpoint
    {
        public static void MapGetSustainabilityReportEndpoint(this IEndpointRouteBuilder endpoint)
        {
            endpoint
                .MapPost(OutgoingDeclarationsEndpointUrls.GET_SUSTAINABILITY_REPORT_ENDPOINT, async (
                    GetSustainabilityReportRequest request,
                ...
                ) =>
                {
                    ...
                    {
                        //If user is not admin we must only get the customers of which it has sustainabilityReport access. If the list is empty we must set it to the list of customer permissions it has to filter by it
                        ...
                        if (request.CustomerIds != null && request.CustomerIds.Length > 0)
                        {
                            customerIdsFilter = request.CustomerIds.Where(c =>
                            {
                                ...
                                return ((customerPermission != null) &&
                                        (customerPermission.Permissions.Contains(CustomerPermission.SustainabilityReport) ||
                        ...
                        else
                        {
                            ...
                            customerIdsFilter = customerPermissions
                                .Where(c => c.Permissions.Contains(CustomerPermission.SustainabilityReport) ||
                                            c.Permissions.Contains(CustomerPermission.Admin))
                        ...
                        if (request.CustomerNumbers != null && request.CustomerNumbers.Length > 0)
                        {
                            ...
                            {
                                var customerPermission = customerPermissions.Where(p => p.CustomerNumber.Value == c)
                                    .FirstOrDefault();

                                return ((customerPermission != null) &&
                                        (customerPermission.Permissions.Contains(
                        ...
                        if (customerIdsFilter.IsEmpty() || customerNumbersFilter.IsEmpty())
                        {
                            ...
                            throw new BusinessException(
                                "A regular user without customers assigned has no transactions");
                        }
                    }
                    ...
                    var filteringParameters = SustainabilityAndFuelConsumptionFilteringParameters.Create(datePeriod,
                        productNamesFilter, customerIdsFilter, customerNumbersFilter, maxColumnsFilter);

                    var query = GetSustainabilityReportQuery.Create(filteringParameters);

                    var results =
                        await queryBus.Send<GetSustainabilityReportQuery, GetSustainabilityReportResponse>(query,
                            cancellationToken);

                    return Results.Ok(results);
                })
                .RequireAuthorization()
                .Produces<GetSustainabilityReportResponse>()
                .WithName("GetSustainabilityReport")
