...
[Collection("End2End")]
public class SustainabilityReportPdfTests
{
    private readonly WebAppFixture webAppFixture;
    private readonly Api api;
    private readonly Fixture any = new();

    public SustainabilityReportPdfTests(WebAppFixture webAppFixture)
    {
    ...
    [Fact]
     public async Task SustainabilityReportPDF_IsCalled()
    {
        ...
        var productNamesQueryDto = new[]{ProductNameEnumeration.Diesel, ProductNameEnumeration.Hvo100}.ToArray();
        ...
        {
            IncomingDeclarationIdPairing.Create(IncomingDeclarationId.Create(incomingDeclarations.First().IncomingDeclarationId.Value), Quantity.Create(1000),
                BatchId.Create(333333333)),
            IncomingDeclarationIdPairing.Create(IncomingDeclarationId.Create(incomingDeclarations.Last().IncomingDeclarationId.Value), Quantity.Create(3000),
                BatchId.Create(22222222))
        };
        var outgoingFilteringParameters = Insight.OutgoingDeclarations.Domain.FilteringParameters.Create(DatePeriod.Create(dateStartGen, dateEndGen), Product.Create("DIESEL"),
            Company.Create("CompanyName"),CustomerName.Create("CustomerName"));
        var outgoingFilteringParameters2 = Insight.OutgoingDeclarations.Domain.FilteringParameters.Create(DatePeriod.Create(dateStartGen, dateEndGen), Product.Create("Hvo100"),
            Company.Create("CompanyName"),CustomerName.Create("CustomerName"));
        var outgoingDeclarations = await OutgoingDeclarationsTestData.SeedWithOutgoingDeclaration(2, outgoingFilteringParameters, webAppFixture, pairings, false);
        var outgoingDeclarations2 = await OutgoingDeclarationsTestData.SeedWithOutgoingDeclaration(2, outgoingFilteringParameters2, webAppFixture, pairings, false);
        ...
        //Act
        await api.GetSustainabilityReportPdf(dateStartQuery, dateEndQuery, productNamesQueryDto, new Guid[] {customerId });
