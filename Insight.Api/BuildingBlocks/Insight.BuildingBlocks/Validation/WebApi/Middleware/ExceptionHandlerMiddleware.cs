...
namespace Insight.BuildingBlocks.Validation.WebApi.Middleware
{
  ...
  //      private readonly ILogger _logger;
        ...
        public ExceptionHandlerMiddleware(RequestDelegate next, IOptions<ExceptionHandlerOption> options)
        {
            _next = next;
            ...
            catch (Exception ex)
            {
                ...
                switch (ex)
                {
                    ...
                    case BusinessException e:
                        var businessError = new BusinessError(e, traceId, instance);
                        httpStatusCode = businessError.Status;
                ...
                if (context.Request.Path.Value != null && context.Request.Path.Value.StartsWith("/administration", StringComparison.OrdinalIgnoreCase))
                {
